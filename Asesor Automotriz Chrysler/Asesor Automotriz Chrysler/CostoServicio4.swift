...
class CostoServicio4: UIViewController,UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate,MFMessageComposeViewControllerDelegate,MFMailComposeViewControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let query = PFQuery(className: "kilometros")
        ...
        query.findObjectsInBackground(block: { (objects : [PFObject]?, error: NSError?) -> Void in
            if error == nil {
                ...
                for object in objects! {
                    ...
                    self.pickerString.add(object["km"] as! String)
                    self.kmTextField.text = (object["km"] as! String)
                }
                
            }
             self.kmPickerView.reloadAllComponents()
             self.kmTextField.text = self.pickerString[0] as? String
        })
    ...
    @IBAction func continuarButton(_ sender: AnyObject) {
        ...
        [self .performSegue(withIdentifier: "costo4a5", sender: self)]
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any!) {
        ...
        if "costo4a5" == segue.identifier {
            let costo5: CostoServicio5 = segue.destination as! CostoServicio5
            
            costo5.receivedNombre = receivedNombre
            costo5.receivedEmail = receivedEmail
            costo5.receivedCelular = receivedCelular
            costo5.receivedTelefono = receivedTelefono
            
            costo5.receivedVehiculo = receivedVehiculo
            costo5.receivedAno = receivedAno
            costo5.receivedKm = kmTextField.text!
