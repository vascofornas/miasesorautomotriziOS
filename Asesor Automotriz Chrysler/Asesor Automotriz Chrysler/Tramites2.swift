...
class Tramites2: UIViewController,UITextFieldDelegate, MFMessageComposeViewControllerDelegate,MFMailComposeViewControllerDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        if textField == emailTextField {
            ...
            celularTextField.resignFirstResponder()
        }
               return true
    }
    ...
    @IBAction func continuarButton(_ sender: AnyObject) {
        ...
        if (!validateEmail(emailCliente!)){
         ...
         [self .performSegue(withIdentifier: "tramite2a3", sender: self)]
     ...
     override func prepare(for segue: UIStoryboardSegue, sender: Any!) {
        ...
        if "tramite2a3" == segue.identifier {
        let tramite3p: Tramites3 = segue.destination as! Tramites3
        
        tramite3p.receivedNombre = nombreTextField.text!
        tramite3p.receivedEmail = emailTextField.text!
        tramite3p.receivedCelular = celularTextField.text!
        tramite3p.receivedTramite = self.receivedTramite
        }
    }
